...
public:
	...
	{
		if (!MmIsAddressValid(reinterpret_cast<PVOID>(Address)))
			return false;
	...
	virtual void TakeDownDetour()
	{
		...
		if (Addr && MmIsAddressValid(reinterpret_cast<PVOID>(Addr)))
		{
			memcpy(reinterpret_cast<PVOID>(Addr), OriginalAsm, 0x10);

			__dcbst(0, reinterpret_cast<PVOID>(Addr));
			__sync();
			__isync();
		}
		...
		ZeroMemory(OriginalAsm, sizeof(OriginalAsm));
	}
